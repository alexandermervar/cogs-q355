...
def get_model(dropout=0.2):
    print('Build model...')
    model = Sequential()
    model.add(Bidirectional(LSTM(128, return_sequences=True), input_shape=(SEQUENCE_LEN, len(words))))
...
if __name__ == "__main__":
    ...
    file_path = "./checkpoints/LSTM_LYRICS-epoch{epoch:03d}-words%d-sequence%d-minfreq%d-" \
                "loss{loss:.4f}-acc-val_loss{val_loss:.4f}-val_acc{val_accuracy:.4f}" % \
